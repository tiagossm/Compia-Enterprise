...
interface LeadConversionModalProps {
    ...
    onClose: () => void;
    onSuccess: () => void;
    lead: any; // Using any for flexibility with Lead type
}
...
export default function LeadConversionModal({
    ...
    // Pre-fill form with Lead data when modal opens or lead changes
    useEffect(() => {
        if (lead && isOpen) {
            setFormData(prev => ({
                ...prev,
                name: lead.nome_fantasia || lead.company_name || '',
                ...
                nome_fantasia: lead.nome_fantasia || lead.company_name || '', // Fallback to company_name

                // Try to map other fields if they exist in lead extended data
            ...
            // If lead already has CNPJ, maybe trigger a lookup or just set success if data looks complete?
            if (lead.cnpj && lead.cnpj.length >= 14) {
                // Optional: could auto-trigger lookup here, but let's leave it manual for now to avoid spam
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            // CALL CONVERSION ENDPOINT INSTEAD OF CREATE
            ...
            if (response.ok) {
                const data = await response.json();
                console.log('Lead convertido com sucesso:', data);

                // Reset form data handled by unmount/remount usually, but clean up just in case
                setCnpjStatus('idle');
                setActiveTab('basic');

                console.log('Chamando onSuccess para refresh...');
                onSuccess(); // Should trigger CRM refresh

                // Small delay to ensure refresh/navigation
                setTimeout(() => {
                    onClose();
                    alert(`Sucesso! Lead convertido em Organização (ID: ${data.organization_id}).`);
                }, 500);
            } else {
                const error = await response.json();
                throw new Error(error.error || 'Erro ao converter lead');
            }
        } catch (error) {
            console.error('Error converting lead:', error);
            alert(error instanceof Error ? error.message : 'Erro ao converter lead. Tente novamente.');
        } finally {
            setLoading(false);
        }
    };

    // Carregar organizações disponíveis (para Parent Org selection)
    useEffect(() => {
    ...
    const getAvailableParentOrganizations = () => {
        // Usar as organizações carregadas via API
        const orgsToFilter = availableOrganizations;
    ...
    return (
        <DialogWrapper
            ...
            description="Revise os dados para criar a nova organização e efetivar a conversão."
            maxWidth="max-w-6xl"
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <p className="text-sm text-slate-600">
                                Transformando "{lead.company_name}" em uma Organização Ativa
                            </p>
                        </div>
                    </div>
                    <button
