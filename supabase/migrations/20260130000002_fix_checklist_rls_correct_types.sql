-- ============================================================================
-- Migration: Fix RLS Policies for Checklist Tables (TIPOS CORRETOS)
-- Date: 2026-01-30
-- Purpose: Add missing RLS policies with correct type handling
--          organization_id = BIGINT, created_by (templates) = TEXT
-- ============================================================================
...
CREATE POLICY "checklist_folders_select" ON public.checklist_folders
    FOR SELECT USING (
        ...
        OR
        -- Users see their organization's folders (organization_id is BIGINT)
        organization_id IN (
...
CREATE POLICY "checklist_templates_select" ON public.checklist_templates
    FOR SELECT USING (
        ...
        OR
        -- Users see their organization's templates (organization_id is BIGINT)
        organization_id IN (
            SELECT organization_id FROM public.users
            WHERE id = COALESCE(auth.uid(), current_user_id())
        )
...
CREATE POLICY "checklist_templates_insert" ON public.checklist_templates
    FOR INSERT WITH CHECK (
        ...
        OR
        -- Users can create in their org (organization_id is BIGINT)
        (
            organization_id IN (
                SELECT organization_id FROM public.users
                WHERE id = COALESCE(auth.uid(), current_user_id())
...
CREATE POLICY "checklist_templates_update" ON public.checklist_templates
    FOR UPDATE USING (
        ...
        OR
        -- Template creator can update their own (created_by is TEXT, need to cast UUID to TEXT)
        created_by = COALESCE(auth.uid(), current_user_id())::text
    );
...
CREATE POLICY "checklist_templates_delete" ON public.checklist_templates
    FOR DELETE USING (
        ...
        OR
        -- created_by is TEXT, cast UUID to TEXT for comparison
        created_by = COALESCE(auth.uid(), current_user_id())::text
    );
